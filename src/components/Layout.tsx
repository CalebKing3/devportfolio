import React, { useState, useEffect, useRef, Fragment, ReactNode, FC } from 'react';
...
import Shortcuts from './Shortcuts'; // Import Shortcuts
...
const LayoutComponent: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    ...
    // Removed duplicate YouTube entry
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--terminal-height', isBottomPanelOpen ? `${terminalHeight}px` : '30px');
  ...
  const getFileName = (path: string) => {
    ...
    switch (path) {
      ...
      case '/youtube': return <Play size={16} className="text-accent-red" />; // Added YouTube icon mapping
    ...
    <div className='relative'>
      ...
      <div className="ide-window">
        ...
        <header className="ide-header">
          <div className="ide-menubar w-full">
            <div className="flex items-center h-full overflow-x-auto w-1/2">
              <div className="ide-app-logo px-2 flex items-center w-1/4">
                <Code size={18} className="text-accent-blue mr-1" />
                <span className="text-editor-text font-semibold">DevPortfolio</span>              </div>
             {navItems.map((item) => (
                  
                <NavLink
                    key={item.path} // Key is now unique
                    to={item.path}
            ...
            <div className="flex items-center h-full flex-shrink-0 w-1/2 justify-end">
              <div className="ide-menubar-item">
                <Search size={16} />
              </div>
              <div className="ide-menubar-item relative">
                  <Bell size={16} />
                <span className="absolute top-1 right-1 w-2 h-2 bg-accent-blue rounded-full"></span>              </div>
                <ThemeToggle/>
              </div>
        ...
        <div className="ide-toolbar w-full">
        <div className="flex items-center space-x-1">
            ...
            <button className="ide-toolbar-btn" title="Layers">
              <Layers size={16} />
            </button>
            <span className="ide-toolbar-separator"></span>
        ...
        <div className="breadcrumbs">
          <div className="breadcrumb-item">
            <span className="text-accent-yellow">portfolio</span></div>
          <div className="breadcrumb-item">
            <span className="text-accent-blue">src</span></div>
          <div className="breadcrumb-item">
            <span className="text-accent-green">pages</span>
          </div>
          <div className="breadcrumb-item">
            <span className="text-editor-text">{getFileName(location.pathname)}</span>
          </div>
        </div>

        {/* Main Content Area */}
        <div className="ide-main flex-1">

          <motion.div key="left-sidebar"
              initial={false}
              animate={{
                width: isSidebarOpen ? 200 : 48,
                x: isSidebarOpen ? 0 : -200,
                transition: { duration: 0.25 },

              }}
              className={`ide-sidebar ${!isSidebarOpen ? 'collapsed' : ''} w-fit`}
            >
        ...
        <div className="flex flex-col flex-1">
            <div className="ide-tabs">
                {navItems.map((item) => (
              <Fragment key={item.path}> {/* Key is now unique */} 
              
                <NavLink
                
                  key={item.path} // Also ensuring key here is unique after fixing navItems
                  to={item.path}
          ...
          {/* Right Sidebar */}
            ...
            <motion.div
              ...
              initial={false}
             animate={{
                width: isStructureSidebarOpen ? 250 : 48,
              }}
              className={`ide-sidebar copilot-sidebar ${!isStructureSidebarOpen ? 'collapsed' : ''} w-fit`}
            >
              {!isStructureSidebarOpen ? (
...
const Layout: FC<{ children: ReactNode }> = ({ children }) => <>{LayoutComponent({ children })}</>;
